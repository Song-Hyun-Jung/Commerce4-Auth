    ...
    stage('Build image') {
      steps{
        script {
          ...
          sh "docker build --no-cache -t 192.168.100.12/commerce-hj/commerce-hj-auth:v$BUILD_NUMBER ."
    ...
    stage('Push Image') {
      ...
      steps{
        script {
          ...
          sh "docker push 192.168.100.12/commerce-hj/commerce-hj-auth:v$BUILD_NUMBER"
          }
        echo "Push Image END"
      }
    }
